...
Automated Initialization Version
"""
...
class CarlaAutowareBridge(Node):
    def __init__(self):
        ...
        # CARLA Connection
        print(f"[BRIDGE] Connecting to CARLA at {self.carla_host}:{self.carla_port}...")
        ...
        print("[BRIDGE] Node started. Monitoring car for initialization...")
    ...
    def _attempt_autoware_initialization(self):
        """Looping call to Autoware to force EKF activation."""
        if self.localization_initialized or not self.ego_vehicle:
            return

        if not self.init_cli.service_is_ready():
            print("[BRIDGE] Waiting for Autoware Initializer Service...")
            return

        # Prepare the request based on current CARLA position
        ...
        # Use the list format for the request pose
        ...
        req.pose = [p] # The API expects a list of poses

        print("[BRIDGE] Requesting Autoware Initialization...")
    ...
    def _init_callback(self, future):
        try:
            ...
            if response.status.success:
                print("[BRIDGE] SUCCESS: Autoware Localization Initialized!")
                self.localization_initialized = True
            else:
                print(f"[BRIDGE] Init Rejected: {response.status.message}")
        except Exception as e:
            print(f"[BRIDGE] Service call failed: {e}")

    def _on_control_command(self, msg):
        if self.ego_vehicle:
            ctrl = carla.VehicleControl()
            ctrl.throttle = min(msg.longitudinal.velocity * 0.2, 1.0)
    ...
    def _publish_vehicle_state(self):
        ...
        # Fix: Deadzone to satisfy "Vehicle not stopped" error
        real_speed = math.sqrt(velocity.x**2 + velocity.y**2 + velocity.z**2)
        reported_speed = 0.0 if real_speed < 0.15 else real_speed

        # 1. TF map -> base_link (Critical for UI display)
        t = TransformStamped()
        ...
        # 4. IMU orientation
        imu = Imu()
        imu.header.stamp, imu.header.frame_id = now, "tamagawa/imu_link"
